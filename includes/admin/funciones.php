<?php
//You still shouldn't be able to open
defined( 'ABSPATH' ) || exit;

//Check if you need the international telephone prefix
function woo_sms_prefijo( $servicio ) {
	$prefijo = [ 
		"adlinks",
		"altiria",
		"bulkgate",
...
function woo_sms_normaliza_mensaje( $mensaje ) {
	$reemplazo = [ 
		'Š'			=> 'S', 
		'š'			=> 's', 
		'Đ'			=> 'Dj', 
...
//encode the message
function woo_sms_codifica_el_mensaje( $mensaje ) {
	return apply_filters( 'woo_sms_message_return', urlencode( html_entity_decode( $mensaje, ENT_QUOTES, "UTF-8" ) ), $mensaje);
}

//Processes the telephone number and adds, if necessary, the prefix
function woo_sms_procesa_el_telefono( $pedido, $telefono, $servicio, $propietario = false, $envio = false ) {
	$telefono_procesado = $telefono;
	
	if ( empty( $telefono_procesado ) ) { //Control
		return apply_filters( 'woo_sms_phone_return', $telefono_procesado, $pedido, $telefono, $servicio, $propietario, $envio );
	}
	
	//Allow other plugins to prevent the phone number from being processed
	if ( apply_filters( 'woo_sms_phone_process', true, $pedido, $telefono, $servicio, $propietario, $envio ) ) {
		...
		$prefijo				= woo_sms_prefijo( $servicio );
		$telefono_procesado		= str_replace( [ '+', '-' ], '', filter_var( $telefono, FILTER_SANITIZE_NUMBER_INT ) );
		if ( substr( $telefono_procesado, 0, 2 ) == '00' ) { //Code proposed by Marco Almeida (https://wordpress.org/support/topic/problems-sending-to-international-numbers-via-plivo/)
		...
		if ( ! $propietario ) {
			if ( ( ! $envio && $billing_country && ( WC()->countries->get_base_country() != $billing_country ) || $prefijo ) ) {
				$prefijo_internacional = woo_sms_dame_prefijo_pais( $billing_country ); //Billing phone
			} else if ( ( $envio && $shipping_country && ( WC()->countries->get_base_country() != $shipping_country ) || $prefijo ) ) {
				$prefijo_internacional = woo_sms_dame_prefijo_pais( $shipping_country ); //shipping phone
			}
		} else if ( $propietario && $prefijo ) {
			$prefijo_internacional = woo_sms_dame_prefijo_pais( WC()->countries->get_base_country() );
		}
	...
	//Allow other plugins to modify the returned phone
	return apply_filters( 'woo_sms_phone_return', $telefono_procesado, $pedido, $telefono, $servicio, $propietario, $envio );
}
...
function woo_sms_procesa_variables( $mensaje, $pedido, $variables, $nota = '' ) {
	...
	$woo_sms_variables = [ //You have to add a hyphen
		"order_key", 
		"billing_first_name", 
		"billing_last_name", 
		"billing_company", 
	...
	foreach ( $busqueda[ 1 ] as $variable ) { 
		...
		if ( !in_array( $variable, $woo_sms ) && !in_array( $variable, $woo_sms_variables ) && !in_array( $variable, $variables_personalizadas ) ) {
			continue;
		}

		$especiales = [  //Special variables (non-standard and non-custom)
			"order_date", 
			"modified_date", 
			"shop_name", 
			"note", 
		...
		if ( ! in_array( $variable, $especiales ) ) {
			if ( in_array( $variable, $woo_sms ) ) {
				$mensaje = str_replace( "%" . $variable . "%", is_callable( [ $pedido, 'get_' . $variable ] ) ? $pedido->{'get_' . $variable}() : $pedido->$variable, $mensaje ); //Standard Variables - Object
			} else if ( in_array( $variable, $woo_sms_variables ) ) {
				$mensaje = str_replace( "%" . $variable . "%", $variables_de_pedido[ "_" . $variable ][ 0 ], $mensaje ); //Variables standard - Array
			} else if ( isset( $variables_de_pedido[ $variable ] ) || in_array( $variable, $variables_personalizadas ) ) {
				$mensaje = str_replace( "%" . $variable . "%", $variables_de_pedido[ $variable ][ 0 ], $mensaje ); //Order and custom variables
			}
			...
			if ( ! isset( $woo_sms_settings[ 'productos' ] ) || $woo_sms_settings[ 'productos' ] != 1 ) {
	...
	//Allow other plugins to modify the returned message
	return apply_filters( 'woo_sms_message' , $mensaje , $numero_de_pedido );
}

//Returns the country prefix code
function woo_sms_dame_prefijo_pais( $pais = '' ) {
	$paises = [ 
		'AC' => '247', 
		'AD' => '376', 
		'AE' => '971', 
